...
`subagent_progress`: the latter is emitted only from real business progress,
while the former is emitted when a visible task starts, completes, or fails.
...
The client groups task lists by `delegation_id`, appends a task only for
`started`, then updates that item by `step_id` for `completed` or `failed`.
It must not pre-create future tasks.
