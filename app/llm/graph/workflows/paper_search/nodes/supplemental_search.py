...
class SupplementalSearchPlannerNode:
    async def __call__(self, state: Mapping[str, Any]) -> dict[str, Any]:
        ...
        for action in sorted(
            ...
            if (
                action.get("strategy") != "next_page"
                or not cursor.get("has_next")
                or source not in plans_by_source
            ):
                continue
            plan = {
                **plans_by_source[source],
                "arguments": dict(plans_by_source[source]["arguments"]),
            }
        ...
        return {
            "stage": "searching",
            "status": "running",
            "active_search_sources": [plan["source"] for plan in active_plans],
            "active_source_query_plans": active_plans,
            "supplemental_search_round": int(state.get("supplemental_search_round", 0))
            + 1,
        }
